...
export async function USAComponent() {
  ...
  return (
    <div className="absolute inset-0">
      <div
        ...
        style={{
          backgroundImage:
            "linear-gradient(180deg, rgba(0,0,0,0.6), rgba(0,0,0,0.3)), url(/images/New-york.jpg)",
        }}
      />

      <div className="absolute inset-0 flex items-center justify-center">
        <div>
          {"The New York City".split("").map((c, i) => (
            <span
              key={i}
              className="char"
              style={{ display: "inline-block", fontSize: "45px", lineHeight: "45px" }}
            >
              {c === " " ? "\u00A0" : c}
            </span>
          ))}
          ...
          <ScrollText duration={2} from={"left"} distance={200}>
            <p className="max-w-225 mt-10 text-white">{t("descriptionNewYork")}</p>
          </ScrollText>
